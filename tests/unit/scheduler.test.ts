...
describe('Scheduler Logic', () => {
  ...
  it('queueActiveSignals adds active signals to the evaluation queue', async () => {
    ...
    expect(mockQueueAdd).toHaveBeenCalledWith('evaluate', { signalId: 'signal-1' }, expect.anything());
    expect(mockQueueAdd).toHaveBeenCalledWith('evaluate', { signalId: 'signal-2' }, expect.anything());
  });
